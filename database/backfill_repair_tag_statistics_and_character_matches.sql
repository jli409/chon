-- =============================================================================
-- Repair user_sessions, tag_statistics, character_matches (question_responses only)
-- =============================================================================
--
...
--
-- tag_statistics: rebuilt from question_responses ONLY when distinct canonical
-- tag_statistics < 6 (delete + insert six rows). Sessions that already have six
-- tag rows but lack character_matches keep existing tag_statistics.
--
-- character_matches: for every target session that has six canonical tag_statistics
-- after the tag step, existing character_matches rows are deleted and six rows
-- are inserted (adjusted core endurance, same as Results.tsx /
-- database/backfill_character_matches_adjusted_core_from_tag_statistics.sql).
--
-- Does NOT read tag_scores.
--
-- BEFORE YOU RUN
...
-- 2) Rebuild tag_statistics only for sessions with incomplete tag_statistics
...
agg AS (
  SELECT
    user_session_id,
    tag_english,
    SUM(pct)::int AS user_score,
    (COUNT(*) * 100)::int AS total_possible_score,
    LEAST(
      100,
      GREATEST(
        0,
        ROUND((SUM(pct::numeric) / NULLIF(COUNT(*) * 100, 0)) * 100)::int
      )
    ) AS score_percentage,
